...
The `colors` module is used for [color mode conversion / color space conversion / color component synthesis and decomposition], etc.  
It also includes some color-related utilities, such as [luminance calculation / similarity comparison], etc.

The `colors` module works together with the [images](image) module to provide more image and color related functionality.

---

## Color Representation

AutoJs6 supports the following ways to represent a color:

- [Color Code (ColorHex)](dataTypes#colorhex)
    - Literals
        - `#RGB` (e.g. `#F00` represents red, equivalent to `#FF0000`)
        - `#RRGGBB` (e.g. `#FF0000` represents red)
        - `#AARRGGBB` (e.g. `#80FF0000` represents semi-transparent red)
    - Methods
        - [colors.toHex](#m-tohex) (e.g. `colors.toHex(0xFF0000)` returns the color string for red, result is `#FF0000`)
        - [colors.toFullHex](#m-tofullhex) (e.g. `colors.toFullHex(0xFF0000)` returns the full color string for red, result is `#FFFF0000`)
        - ... ...
- [Color Integer (ColorInt)](dataTypes#colorint)
    - Literals
        - `0xAARRGGBB` (e.g. `0x8000FF00` within Java's `Integer` range represents semi-transparent green)
    - Methods
        - [colors.rgb](#m-rgb) (e.g. `colors.rgb(255, 0, 0)` represents red)
        - [colors.argb](#m-argb) (e.g. `colors.argb(128, 255, 0, 0)` represents semi-transparent red)
        - [colors.rgba](#m-rgba) (e.g. `colors.rgba(255, 0, 0, 128)` represents semi-transparent red)
        - [colors.hsv](#m-hsv) (e.g. `colors.hsv(0, 1, 1)` represents red)
        - [colors.hsva](#m-hsva) (e.g. `colors.rgba(0, 1, 1, 0.5)` represents semi-transparent red)
        - [colors.hsl](#m-hsl) (e.g. `colors.hsl(0, 1, 0.5)` represents red)
        - [colors.hsla](#m-hsla) (e.g. `colors.hsl(0, 1, 0.5, 0.5)` represents semi-transparent red)
        - [colors.toInt](#m-toint) (e.g. `colors.toInt('#FF0000')` returns the color integer for red, result is `-65536`)
        - ... ...
    - Constants
        - [colors.android.RED](#p-android) (Red color integer from the [Android Color List](colorTable#Android-颜色列表))
        - [colors.android.BLACK](#p-android) (Black color integer from the [Android Color List](colorTable#Android-颜色列表))
        - ... ...
        - [colors.css.RED](#p-css) (Red color integer from the [CSS Color List](colorTable#CSS-颜色列表))
        - [colors.css.BLACK](#p-css) (Black color integer from the [CSS Color List](colorTable#CSS-颜色列表))
        - ... ...
        - [colors.web.RED](#p-web) (Red color integer from the [Web Color List](colorTable#WEB-颜色列表))
        - [colors.web.BLACK](#p-web) (Black color integer from the [Web Color List](colorTable#WEB-颜色列表))
        - ... ...
        - [colors.material.ORANGE](#p-material) (Orange color integer from the [Material Color List](colorTable#Material-颜色列表))
        - [colors.material.ORANGE_300](#p-material) (Orange color integer with shade 300 from the [Material Color List](colorTable#Material-颜色列表))
        - ... ...
        - [colors.RED](#p-red) (Red color integer from the [Combined Color List](colorTable#融合颜色列表))
        - [colors.BLACK](#p-black) (Black color integer from the [Combined Color List](colorTable#融合颜色列表))
        - [colors.ORANGE](#p-orange) (Orange color integer from the [Combined Color List](colorTable#融合颜色列表))
        - ... ...
- [Color Components Array (ColorComponents)](dataTypes#colorcomponents)
    - Methods
        - [colors.toRgb](#m-torgb) (Color components array `[R, G, B]`)
        - [colors.toRgba](#m-torgba) (Color components array `[R, G, B, A]`)
        - [colors.toArgb](#m-toargb) (Color components array `[A, R, G, B]`)
        - [colors.toHsv](#m-tohsv) (Color components array `[H, S, V]`)
        - [colors.toHsva](#m-tohsva) (Color components array `[H, S, V, A]`)
        - [colors.toHsl](#m-tohsl) (Color components array `[H, S, L]`)
        - [colors.toHsla](#m-tohsla) (Color components array `[H, S, L, A]`)
        - ... ...
- [Color Name (ColorName)](dataTypes#colorname)
    - Constants
        - "red" (red)
        - "black" (black)
        - "orange" (orange)
...
Special attention: The color string for black is `#000000`, which is the shorthand form of the full color string `#FF000000`. Its ARGB components are represented as `argb(255, 0, 0, 0)`.

Therefore, the color integer for black is not `0`, but `-16777216`.

The color integer `0` corresponds to fully transparent color, i.e. `#00000000`, with ARGB components `argb(0, 0, 0, 0)`.
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- <ins>**returns**</ins> { [ColorInt](dataTypes#colorint) } - Color integer

Converts the color parameter to a [color integer (ColorInt)](dataTypes#colorint).
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- <ins>**returns**</ins> { [ColorHex](dataTypes#colorhex) } - Color code

Converts the color parameter to a [color code (ColorHex)](dataTypes#colorhex).
...
colors.toHex('#FFCC5500'); /* #CC5500, A (alpha) component omitted. */
...
When the `A (alpha)` component is `100% (255/255;100/100)`, `FF` is automatically omitted.  
For example, `#FFC0C0C0` will be automatically converted to `#C0C0C0`. This method is equivalent to `toHex(color, 'auto')`.
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- **[ alpha = `'auto'` ]** { [boolean](dataTypes#boolean) | `'keep'` | `'none'` | `'auto'` } - A (alpha) component parameter
- <ins>**returns**</ins> { [ColorHex](dataTypes#colorhex) } - Color code

Converts the color parameter to a [color code (ColorHex)](dataTypes#colorhex) and determines the display state of the `A (alpha)` component in the color code according to the `alpha` parameter.

`A (alpha)` component parameter values:

| Value                                                    | Meaning                                                                 | Default |
|----------------------------------------------------------|-------------------------------------------------------------------------|:-------:|
| <span style="white-space:nowrap">'keep' / true</span>    | <span style="white-space:nowrap">Force display of A component, regardless of whether A is 0xFF</span> |         |
| <span style="white-space:nowrap">'none' / false</span>   | <span style="white-space:nowrap">Force removal of A component, keep only R / G / B components</span> |         |
| <span style="white-space:nowrap">'auto'</span>           | <span style="white-space:nowrap">Automatically decide display state based on whether A is 0xFF</span> | √       |
...
colors.toHex(cA, 'auto'); /* #AAC0C0C0, 'auto' parameter can be omitted. */
colors.toHex(cB, 'auto'); /* #C0C0C0, 'auto' parameter can be omitted. */
colors.toHex(cC, 'auto'); /* #C0C0C0, 'auto' parameter can be omitted. */

/* cA discards A component. */
colors.toHex(cA, false); // #C0C0C0
colors.toHex(cA, 'none'); /* Same as above. */

/* cB keeps A component. */
colors.toHex(cB, true); // #FFC0C0C0
colors.toHex(cB, 'keep'); /* Same as above. */

/* cC forces display of A component. */
colors.toHex(cC, true); // #FFC0C0C0
colors.toHex(cC, 'keep'); /* Same as above. */
```
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- **length** { `8` | `6` | `3` } - Hex code length parameter
- <ins>**returns**</ins> { [ColorHex](dataTypes#colorhex) } - Color code

Converts the color parameter to a [color code (ColorHex)](dataTypes#colorhex) and determines the display format of the color code according to the `length` parameter.

Hex code length parameter values:

| Value | Meaning                                                              |
|-------|----------------------------------------------------------------------|
| 8     | Force display of A component, result format #AARRGGBB                |
| 6     | Force removal of A component, result format #RRGGBB                  |
| 3     | Force removal of A component, result format #RGB                     |
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- <ins>**returns**</ins> { [ColorHex](dataTypes#colorhex) } - Full form of the color code

Forces conversion of the color parameter to the full form of [color code (ColorHex)](dataTypes#colorhex) (#AARRGGBB).

This method is an alias for [colors.toHex(color, 8)](#tohexcolor-length).
...
- **[ color = `Colors.BLACK` ]** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- <ins>**returns**</ins> { [Color](colorType) } - Color instance

Builds a [Color](colorType) instance, equivalent to `new Color(color?)` or `Color(color?)`.

```js
colors.build('dark-orange') /* Build Color instance with dark orange */
    .setAlpha(0.85) /* Set alpha to 85%. */
    .removeBlue() /* Remove B (blue) component. */
    .toHex(); // #D9FF8C00

/* Build empty Color instance, set HSLA components and convert to Hex code. */
...
Builds a [Color](colorType) instance, equivalent to `new Color(red, green, blue, alpha?)`.

```js
colors.build(120, 60, 240).setAlpha(0.85).toHex(); // #D9783CF0
colors.build(120, 60, 240, 0.85).toHex(); /* Same as above. */
colors.build().setRgba(120, 60, 240, 0.85).toHex(); /* Same as above. */
```
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- <ins>**returns**</ins> { [string](dataTypes#string) } - Color summary

Gets a color summary.

The format is `hex($HEX), rgba($R,$G,$B/$A), int($INT)`.

The `A (alpha)` component is displayed in the `0..1` range, with at least one decimal place and at most two:

| Component Value | Displayed Value |
|-----------------|-----------------|
| 0               | 0.0             |
| 1               | 1.0             |
| 0.64            | 0.64            |
...
- **color** { [string](dataTypes#string) } - Color parameter
- <ins>**returns**</ins> { [ColorInt](dataTypes#colorint) } - Color integer

Converts the color parameter to a [color integer (ColorInt)](dataTypes#colorint).

Similar to [toInt](#m-toint), but the accepted parameter range is relatively narrower and the type/value requirements are stricter.  
The color parameter for parseColor only supports 6-digit and 8-digit color codes and a limited set of color names.
...
The table below compares some results between toInt and parseColor:

| Parameter                 | toInt     | parseColor     |
|---------------------------|-----------|----------------|
| 'blue'                    | -16776961 | -16776961      |
| 'burnt-orange'            | -3386112  | # throws error # |
| '#FFCC5500'               | -3386112  | -3386112       |
| '#CC5500'                 | -3386112  | -3386112       |
| '#C50'                    | -3386112  | # throws error # |
| 0xFFCC5500                | -3386112  | # throws error # |
| colors.web.BURNT_ORANGE   | -3386112  | # throws error # |

Unless multi-version compatibility is required, it is recommended to always use toInt instead of parseColor.
...
Alias for [toHex(color)](#tohexcolor).
...
Converts the color parameter to a [color code (ColorHex)](dataTypes#colorhex) and determines the display state of the `A (alpha)` component based on the `alpha` parameter.

Alias for [toHex(color, alpha)](#tohexcolor-alpha).
...
Converts the color parameter to a [color code (ColorHex)](dataTypes#colorhex) and determines the display state based on the `length` parameter.

Alias for [toHex(color, length)](#tohexcolor-length).
...
Gets the `A (alpha)` component of the color, range `[0..255]`.
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- **options** {{
    - [ max = `255` ]?: `1` | `255` - Maximum value of the range
- }} - Options parameter
- <ins>**returns**</ins> { [IntRange[0..1]](dataTypes#intrange) | [IntRange[0..255]](dataTypes#intrange) }

Gets the `A (alpha)` component of the color.

The range is `[0..1]` (when `options.max` is `1`) or `[0..255]` (when `options.max` is `255` or omitted).

```js
colors.alpha('#663399', { max: 1 }); // 1
colors.alpha('#663399', { max: 255 }); // 255
colors.alpha('#663399'); /* Same as above. */

colors.alpha('#05060708', { max: 1 }); // 0.0196078431372549
colors.alpha('#05060708', { max: 255 }); // 5
colors.alpha('#05060708'); /* Same as above. */
```

When `options.max` is `1`, this is equivalent to the [colors.alphaDouble](#m-alphadouble) method.
...
Gets the `A (alpha)` component of the color, range `[0..1]`.

Equivalent to `colors.alpha(color, { max: 1 })`.
...
colors.alpha('#05060708', { max: 1 }); /* Same as above. */
```
...
Alias for [colors.alpha(color)](#m-alpha).
...
Alias for [colors.alpha(color, options)](#m-alpha).
...
Alias for [colors.alphaDouble(color)](#m-alphadouble).
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- **alpha** { [ColorComponent](dataTypes#colorcomponent) } - Color component - A (alpha)
- <ins>**returns**</ins> { [ColorInt](dataTypes#colorint) }

Sets the `A (alpha)` component of the color and returns the new color as a color integer.

```js
colors.toHex(colors.setAlpha('#663399', 0x80)); // #80663399
colors.toHex(colors.setAlpha('#663399', 0.5)); /* Same as above; 0.5 is interpreted as percentage (50%). */

colors.toHex(colors.setAlpha('#663399', 255)); // #FF663399
colors.toHex(colors.setAlpha('#663399', 1)); /* Same as above; 1 is interpreted as percentage (100%) by default. */
```
...
- **color** { [OmniColor](omniTypes#omnicolor) } - Color parameter
- **percentage** { [ColorComponent](dataTypes#colorcomponent) } - Relative percentage
- <ins>**returns**</ins> { [ColorInt](dataTypes#colorint) }

Sets a relative percentage for the `A (alpha)` component and returns the new color as a color integer.

If the current color's `A (alpha)` component is `80` and you want to set the `A` component to `50%` of that (i.e. `40`):

```js
colors.setAlphaRelative(color, 0.5);
colors.setAlphaRelative(color, '50%'); /* Same effect as above. */
```

Similarly, if you want to set the `A` component to `1.5` times the original (i.e. `120`):
...
If the resulting value exceeds `255`, it will be clamped to `255`:

```js
colors.setAlphaRelative(color, 10); /* Final A component value is 255, not 800. */
```

Note that if the original color's `A` component is `0`, the `A` component will remain `0` no matter what relative value is set.
...
Removes the `A (alpha)` component from the color and returns the new color as a color integer.
...
Gets the `R (red)` component of the color, range `[0..255]`.
...
Gets the `R (red)` component of the color.
...
colors.red('#663399'); /* Same as above. */
```

When `options.max` is `1`, this is equivalent to the [colors.redDouble](#m-reddouble) method.
...
Gets the `R (red)` component of the color, range `[0..1]`.

Equivalent to `colors.red(color, { max: 1 })`.
...
Alias for [colors.red(color)](#m-red).
...
Alias for [colors.red(color, options)](#m-red).
...
Alias for [colors.redDouble(color)](#m-reddouble).
...
Sets the `R (red)` component of the color and returns the new color as a color integer.

```js
colors.toHex(colors.setRed('#663399', 0x80)); // #803399
colors.toHex(colors.setRed('#663399', 0.5)); /* Same as above; 0.5 is interpreted as percentage (50%). */

colors.toHex(colors.setRed('#663399', 255)); // #FF3399
colors.toHex(colors.setRed('#663399', 1)); /* #013399 — different from above. 1 is interpreted as integer component by default, not 100%. */
```
...
Sets a relative percentage for the `R (red)` component and returns the new color as a color integer.

If the current color's `R (red)` component is `80` and you want to set the `R` component to `50%` of that (i.e. `40`):

```js
colors.setRedRelative(color, 0.5);
colors.setRedRelative(color, '50%'); /* Same effect as above. */
```

Similarly, if you want to set the `R` component to `1.5` times the original (i.e. `120`):
